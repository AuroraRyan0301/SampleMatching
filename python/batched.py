...
class _VoxelBatchedRenderOp(dr.CustomOp):
    """
    Batched (i.e. ray-centric) alternative to `mi._RenderOp`.
    """
    ...
    def eval(self, scene, sensors, sensors_idx, sampled_pixels, film_size, batch_samplers,
        ...
        # get voxel center position from params
        # voxel_params = mi.traverse(scene)
        bbox_min = scene.bbox().min
        bbox_max = scene.bbox().max
        # calculate scale and translate
        scaling = (bbox_max - bbox_min)
        translate = 1.0 * bbox_min
        ...
        # import ipdb;ipdb.set_trace()
        dr_grid = mi.cuda_ad_rgb.Point3f(flat.astype(np.float32))  # shape -> (32, 32, 32, 3)

        voxel_center_pos = (dr_grid+0.5) / mi.Float(nx) * scaling + translate
        # import ipdb;ipdb.set_trace()

        with dr.suspend_grad():
            # Sample a set of rays for primal rendering
            rays, ray_weight, pos = sample_batch_grids_by_rays(
                self.sampled_sensors, self.sampled_pixels,
                self.film_size, self.batch_samplers[1], self.spp[0], voxel_center_pos
            )
...
def render_batch_l2loss(batch_size: int,
                 scene: mi.Scene,
                 sensors: mi.SensorPtr,
                 film_size: mi.ScalarVector2u,
                 params: Any = None,
    ...
    return dr.custom(_BatchedRenderOp, scene, sensors, sensors_idx, sampled_pixels, film_size,
                     batch_samplers, params, integrator, film, pixel_format, sampler,
                     (seed+114514, seed_grad+114514), (spp, spp_grad))

def render_batch_voxel(batch_size: int,
                 scene: mi.Scene,
                 sensors: mi.SensorPtr,
                 film_size: mi.ScalarVector2u,
                 params: Any = None,
    ...
    return dr.custom(_VoxelBatchedRenderOp, scene, sensors, sensors_idx, sampled_pixels, film_size,
                     batch_samplers, params, integrator, film, pixel_format, sampler,
                     (seed, seed_grad), (spp, spp_grad))
...
def sample_batch_grids_by_rays(sampled_sensors, sampled_pixels, film_size, sampler, spp, voxel_center_pos):
    """Assumes that all sensors have the same film dimensions."""
    batch_size = dr.width(sampled_pixels)
