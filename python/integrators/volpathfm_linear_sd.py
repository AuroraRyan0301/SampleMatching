...
class VolpathFMLinearSDIntegrator(mi.ad.integrators.common.RBIntegrator):
    ...
    def __init__(self, props=mi.Properties()):
        ...
        self.n_samples_transmittance = props.get('n_samples_transmittance', 1)
    ...
    def sample(self,
        ...
        while loop(active):
            ...
            if not primal:
                ...
                #     self.backpropagate_inscattering_drt(
                #         scene, medium, alt_sampler, ray, si, throughput, depth,
                #         channel, 0.0, active, drt_reservoir=drt_reservoir)
                #     # del adjoint
                ...
                if self.use_drt_mis:
                    ...
                    with dr.resume_grad():
                        dr.backward_from(drt_mis_weight * δL * albedo
                                         * Li)
                    del Li, drt_mis_weight
            # ----------
            ...
            if not primal:
                ...
                # transmittance adjoint
                adj_weight1 = δL * result

                # scattering adjoint
                adj_weight2 = δL * throughput

                self.backpropagate_transmittance(nee_dir_sample, scene, medium, channel, alt_sampler, si, mei, ray,
                                                adj_weight1, adj_weight2, result, throughput, did_scatter, did_escape, depth, drt_reservoir=drt_reservoir,n_samples=self.n_samples_transmittance)
                del adj_weight2, adj_weight1
            # ----------
            ...
            if self.use_nee:
                ...
                nee_contrib = self.sample_emitter_for_nee(nee_dir_sample,
        ...
        if (not primal) and self.use_drt and self.use_drt_subsampling:
            ...
            self.backpropagate_indir_scattering_drt(
                scene, medium, alt_sampler, drt_state.ray(), drt_state.si(),
                None, drt_state.depth(), channel,
                adjoint, drt_state.active(), drt_state.mei(), drt_state.mei_t())
    ...
    def sample_emitter_for_nee(self, nee_dir_sample, mei, scene, sampler, medium, channel,
                               phase_ctx, phase, throughput, active, primal=True, δL=None):
        if not primal:
            nee_sampler = sampler.clone()

        emitted, ds = self.sample_emitter(nee_dir_sample, mei, scene, sampler, medium, channel, active)
        ...
        if not primal:
            ...
            self.sample_emitter(nee_dir_sample, mei, scene, nee_sampler, medium, channel, active,
                                adjoint=adjoint)

        return nee_contrib


    def sample_emitter(self, nee_dir_sample, ref_interaction, scene, sampler, medium, channel, active,
                       adjoint=None):
        """
        ...
        ds, emitter_val = scene.sample_emitter_direction(ref_interaction,  dr.select(active, nee_dir_sample, 0.0),
                                                         False, active)
    ...
    def backpropagate_indir_scattering_drt(self, scene, medium, alt_sampler, ray, si,
                                     throughput, depth, channel,
                                     adjoint, active, mei_sub, interval_to_main_scattering):
        """
        ...
        with dr.resume_grad():
            ...
            to_backward = dr.select(active, mei_sub.sigma_t * dr.detach(albedo_sub), 0.)
            to_backward_albedo = dr.select(active, dr.detach(mei_sub.sigma_t) * albedo_sub, 0.)
            dr.backward_from(interval_to_main_scattering *
                             adjoint * to_backward * drt_Li + drt_mis_weight * interval_to_main_scattering * adjoint * to_backward_albedo * drt_Li)

    def backpropagate_transmittance(self, nee_dir_sample, scene, medium, channel, alt_sampler, si, mei, ray,
                                    adj_weight1, adj_weight2, results, throughput, did_scatter, did_escape, depth, drt_reservoir, n_samples=1):
        ...
        within_depth = ((depth+1) < self.max_depth) & active
        ...
        sub_ray.maxt[active] = dr.select(
            dr.isfinite(si.t), si.t, dr.largest(mi.Float))
        with dr.suspend_grad():
            nee_Li = self.sample_nee(nee_dir_sample,
                scene, alt_sampler, medium, sub_ray, si,
                mei_sub, channel, depth, within_depth)
        with dr.resume_grad():
            _, _, sigma_t_sub = medium.get_scattering_coefficients(mei_sub, active)
            # The higher sigma_t, the lower the transmittance
            if self.use_drt_mis:
                # Note: MIS weight hardcoded for the power heuristic
                drt_mis_weight = 1 / (1 + dr.sqr(dr.detach(sigma_t_sub)))
            else:
                drt_mis_weight = 1.0
            contribs -= sigma_t_sub * adj_weight1

            albedo_sub = medium.get_albedo(mei_sub, active)

            to_backward = dr.select(active, sigma_t_sub * dr.detach(albedo_sub), 0.)
            to_backward_albedo = dr.select(active, dr.detach(sigma_t_sub) * albedo_sub, 0.)
            contribs +=  adj_weight2 * to_backward * nee_Li + drt_mis_weight * adj_weight2 * to_backward_albedo * nee_Li
        ...
        inv_pdf = interval / self.n_samples_transmittance
        with dr.resume_grad():
            contribs = dr.select(active, contribs, 0.)
            dr.backward_from(contribs * inv_pdf)
        # multiple NEE samples for scattering gradient estimation
        self.backpropagate_grad_multiple_nee(nee_dir_sample, scene, medium, channel, alt_sampler, mei, interval, ray,
                                        adj_weight1, adj_weight2, active, within_depth)

    def backpropagate_grad_multiple_nee(self, nee_dir_sample, scene, medium, channel, alt_sampler, mei, interval, ray,
                                    adj_weight1, adj_weight2, active, within_depth):
        if self.n_samples_transmittance <= 1:
            return
        
        mei_sub = mi.MediumInteraction3f(mei)
        contribs2 = mi.Spectrum(0.)
        phase_ctx = mi.PhaseFunctionContext(alt_sampler)
        phase = mei_sub.medium.phase_function()
        phase[~active] = dr.zeros(mi.PhaseFunctionPtr, 1)
        for _ in range(self.n_samples_transmittance-1):
            mei_sub.t = alt_sampler.next_1d(active) * interval
            mei_sub.p = ray(mei_sub.t)
            with dr.suspend_grad():

                drt_Li = self.sample_emitter_for_nee(nee_dir_sample,
                    mei_sub, scene, alt_sampler, medium, channel, phase_ctx,
                    phase, mi.Spectrum(1.0), within_depth)

                drt_Li = dr.select(within_depth, dr.detach(drt_Li), 0.)
            ...
            with dr.resume_grad():
                ...
                if self.use_drt_mis:
                    ...
                    drt_mis_weight = 1.0
                contribs2 -= sigma_t_sub * adj_weight1

                # here medium.get_albedo(mei_sub, active) can not work multiple times?
                albedo_sub = mei_sub.medium.get_albedo(mei_sub, active)

                to_backward = dr.select(active, sigma_t_sub * dr.detach(albedo_sub), 0.)
                to_backward_albedo = dr.select(active, dr.detach(sigma_t_sub) * albedo_sub, 0.)
                contribs2 += adj_weight2 * to_backward * drt_Li + drt_mis_weight * adj_weight2 * to_backward_albedo * drt_Li
        del mei_sub

        # Probability of sampling each of the new distances
        inv_pdf = interval / self.n_samples_transmittance
        with dr.resume_grad():
            contribs2 = dr.select(active, contribs2, 0.)
            dr.backward_from(contribs2 * inv_pdf)

    def sample_indirect(self, scene, alt_sampler, medium, ray, si, mei, channel,
                         depth, active):
        """
    ...
    def sample_nee(self, nee_dir_sample, scene, alt_sampler, medium, ray, si, mei, channel,
                         depth, active):
        """
        Trace a detached recursive ray to estimate Li incident to the current medium
        interaction.
        ...
        if self.use_nee:
            result += self.sample_emitter_for_nee(nee_dir_sample,
...
class PathState():
    ...
    def __init__(self, depth=None, si=None, last_scatter_direction_pdf=None,
                 escaped=None, active=None, mei_t=None, mei=None, n=None):
        # TODO: would need medium pointer as well if we didn't assume a single medium
        if n is not None:
            assert (depth, si, last_scatter_direction_pdf, escaped, mei_t, mei) == (None, None, None, None, None, None)
            ...
            self._mei_t = dr.zeros(mi.Float, n)
            self._mei = dr.zeros(mi.MediumInteraction3f, n)
        else:
            self._depth = depth
            self._si = si
    ...
    def loop_put(self, loop):
        loop.put(lambda: (self._depth, self._si, self._last_scatter_direction_pdf,
                          self._escaped, self._active, self._mei_t, self._mei))
    ...
    def set(self, state: dict, enabled):
        ...
        self._mei_t[enabled] = state['mei_t']
        self._mei[enabled] = state['mei']
    ...
    def mei_t(self):
        return mi.Float(self._mei_t)
    def mei(self):
        return mi.MediumInteraction3f(self._mei)
...
class DRTReservoir():
    ...
    def update(self, state: dict, weight, sample, active):
        ...
        weight = dr.select(active, dr.mean(dr.detach(weight)), 0)
...
mi.register_integrator("volpathfm_linear_sd", lambda props: VolpathFMLinearSDIntegrator(props))
