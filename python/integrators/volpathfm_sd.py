...
class VolpathFMSDIntegrator(mi.ad.integrators.common.RBIntegrator):
    ...
    def backpropagate_transmittance(self, nee_dir_sample, scene, medium, channel, alt_sampler, si, mei, ray,
        ...
        with dr.suspend_grad():
            nee_Li, indirect_Li = self.sample_recursive_decompose(nee_dir_sample,
        ...
        with dr.resume_grad():
            ...
            contribs += (drt_mis_weight * to_backward_albedo + to_backward) * adj_weight2 * (nee_Li + self.n_samples_transmittance * indirect_Li)
        del mei_sub
    ...
    def backpropagate_grad_multiple_nee(self, nee_dir_sample, scene, medium, channel, alt_sampler, mei, interval, ray,
        ...
        for _ in range(self.n_samples_transmittance-1):
            ...
            with dr.resume_grad():
                ...
                to_backward = dr.select(active, sigma_t_sub * dr.detach(albedo_sub), 0.)
                to_backward_albedo = dr.select(active, dr.detach(sigma_t_sub) * albedo_sub, 0.)
                contribs2 += (drt_mis_weight * to_backward_albedo + to_backward) * adj_weight2 * drt_Li
        del mei_sub
    ...
    def sample_recursive_decompose(self, nee_dir_sample, scene, alt_sampler, medium, ray, si, mei, channel,
                         depth, active):
        """
        Trace a detached recursive ray to estimate Li incident to the current medium
        interaction.
        """
        nee_result = dr.zeros(mi.Spectrum)
        phase_ctx = mi.PhaseFunctionContext(alt_sampler)
        phase = medium.phase_function()

        # 1. Emitter sampling (including the MIS weight)
        if self.use_nee:
            nee_result += self.sample_emitter_for_nee(nee_dir_sample,
        ...
        return nee_result & active, Li & active
...
mi.register_integrator("volpathfm_sd", lambda props: VolpathFMSDIntegrator(props))
